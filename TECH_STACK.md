...
## 既知の制限事項：特定IC区間で不自然な迂回ルートになる（鳴門IC等）

「現在地(徳島)→亀岡駅」や「鳴門IC→垂水IC」のようなIC指定ルートで、明らかに不自然な迂回（一度反対方向の鳴門JCT方面へ進んでから降りて、下道を経由して改めて乗り直す等）が発生する事例が報告された。

**切り分けの経緯**：
1. `exclude=motorway`（高速道路を使わない）を指定した場合に本州⇔四国間で船（南海フェリー、徳島⇔和歌山）のルートが提案される現象を確認。これは、四国と本州を結ぶ橋（大鳴門橋・明石海峡大橋・瀬戸大橋等）がすべて高速道路として分類されており、高速を除外すると陸路が存在しないため、地理的に必然の挙動と判断（バグではない）
2. 鳴門IC→垂水ICの区間で、鳴門JCT方面へ一度迂回してから改めて神戸方面へ入り直す不自然なルートを確認。Mapbox Directionsの`bearings`パラメータ（進みたい方角のヒント）を±90度・±20度で指定して検証したが、結果は変わらなかった
3. `alternatives=true`で代替ルートの有無を確認したが、候補は1つのみで、迂回ルート以外の選択肢自体が存在しなかった
4. 鳴門ICと鳴門JCTは物理的に0.2kmしか離れておらず、実際の運転では一続きの動作のはずだが、Mapboxが参照する道路データ（OpenStreetMapベース）上でこの接続区間が正しく表現されていない可能性が高いと判断

**結論**：この問題はアプリのコードの書き方やAPIの呼び出しオプションでは解決できない。根本原因はMapboxの道路データ（OpenStreetMap由来）側にある可能性が高く、修正するにはOpenStreetMap自体の編集・Mapbox側への反映（数週間〜数ヶ月かかる場合がある）が必要になる。

**現時点の対応**：修正は行わず、既知の制限事項として記録し、優先度を下げて保留する。
