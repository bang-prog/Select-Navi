...
## DynamoDBの費用について（アプリを大きくしていく上での目安）

ルート選択実績を記録する2つ目のテーブル（`SelectNaviRouteChoices`）を追加するにあたり、「データベースが大きくなると費用がかかるのでは」という懸念について事前に調査した内容を記録しておく。

**容量について**：DynamoDBは「容量を使い切って動かなくなる」という心配はほぼ不要。1テーブルに入れられるデータ量は事実上無制限（1項目あたり400KBの上限のみ）で、テーブル全体としてはペタバイト級までスケールする設計になっている。

**料金の仕組み**：現在の利用形態は「オンデマンドモード」の従量課金で、月額固定費はない。課金対象は3つ。

- 書き込み：東京リージョンで目安 約$1.43 / 100万回
- 読み込み：目安 約$0.29 / 100万回
- 保存容量：$0.25〜0.29 / GB・月（最初の25GBは無料枠）

（2024年11月に大幅値下げされているため、正確な最新額は[AWS公式のDynamoDB料金ページ](https://aws.amazon.com/jp/dynamodb/pricing/)で確認するのが確実）

**現実的な費用感の試算**：例えば「1日500人が使い、1人あたり検索5回・地図更新（通報取得）20回」程度の規模を想定すると、書き込み・読み込みともに月々数十円〜百円程度に収まる。月間アクティブ数千人規模になっても、月額数百円〜千円程度が現実的な見立てで、**データベースのコストで問題になる可能性は当面低い**。

**むしろ注意すべき既存のコード上の懸念点**：`web/app/api/reports/route.ts`の通報取得（GET）で`ScanCommand`（テーブル全件スキャン）を使っている。これは「テーブルに何件データがあっても、毎回全件読み込んでからアプリ側で絞り込む」処理のため、通報件数・アクセス頻度が増えるとここの読み込みコストだけが不釣り合いに膨らむ。ユーザーが増えてきたタイミングで、位置情報を使った`Query`（地理的な絞り込み）への切り替えを検討する。

**安全策**：「予算アラートの設定（安全対策）」（本ドキュメント冒頭）で設定済みのゼロ支出予算に加え、想定外の急な費用増加に早く気づけるよう、必要に応じて閾値付きの予算アラート（例：月$5超えたら通知）も検討するとよい。
